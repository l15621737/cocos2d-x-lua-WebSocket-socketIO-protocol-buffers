...
static int
_peek_message_enums(lua_State *L) {
	struct pbc_env * env = (struct pbc_env *)checkuserdata(L, 1);
	size_t sz = 0;
	const char* enum_type = luaL_checklstring(L, 2, &sz);
	void* msg = (void*)pbcP_get_message(env, enum_type);
	lua_newtable(L);
	...
	while(1)
	{
		t = pbc_message_next(msg, &key);
		if (NULL == key)
		{
			break;
		}
		// add table
		lua_pushstring(L, key); // tab key
		lua_pushinteger(L,t);  // tab key t
		lua_rawset(L, -3);     // tab -- tab[key] = t
	}
	/*
	const char *key = NULL;
	struct value * v = (struct value *)_pbcM_sp_next(msg->id, key);
	if (*key == NULL) {
		return 0;
	}*/
...
/*
	...
	:6 integer len(when arg 5 is userdata)
	:6 integer start( when arg 5 is string, optionable, like string.sub(string,start,end))
	:7 integer end(when arg 5 is string,optionable, like string.sub(string,start,end))
	table
 */
static int
_decode(lua_State *L) {
	...
	if (lua_type(L,5) == LUA_TSTRING) {
		...
		int start = luaL_optinteger(L, 6, 1);
		int end = luaL_optinteger(L, 7, -1);

		if (start < 0) start += (slice.len + 1);
		if (end < 0) end += (slice.len + 1);
		...
		if (start-1 > end)
		{
			luaL_error(L, "_decode, must end+1 >= start ");
		}

		slice.buffer = (void*)((char*)slice.buffer +start - 1);
...
luaopen_protobuf_c(lua_State *L) {
	luaL_Reg reg[] = {
		...
		// add by Augustine 7/17/2018
		{"_peek_message_enums",_peek_message_enums },
		{NULL,NULL},
	};
