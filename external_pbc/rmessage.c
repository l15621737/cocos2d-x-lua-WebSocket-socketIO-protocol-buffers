...
// void* is _message
void* 
pbcP_get_message(struct pbc_env * p, const char *name)
{
	return (void*)_pbcP_get_message(p, name);
}
 
int  pbc_message_next(void* pmessage, const char **key)
{
	struct _field * f = (struct _field *)_pbcM_sp_next( ((struct _message*) pmessage)->name, key);
	if (*key == NULL) {
		return 0;
	}
	return _pbcP_type(f, NULL);
}
