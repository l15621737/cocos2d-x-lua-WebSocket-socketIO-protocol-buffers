...
--add by Augustine 9/29/2018

function _reader:null(key)
	return nil
end
----------------------
...
function encode( message, t , func , ...)
	local encoder = c._wmessage_new(P, message,message)
	if encoder == nil then  
		assert(false , "Proto:"..message.." "..c._last_error(P))
	end
...
-- add by Augustine 2018/7/18
local function setdefaultmetatable(rootmeta)
	
	local function _default_metatable(retmeta)
		local t =
		{
			__index = function(tb, key)
				--dump(retmeta)
				local ret = retmeta[key] 
				if 'table' ~= type(ret) then
					rawset(tb, key, ret)
				else
					local mt = _default_metatable(ret)
					ret = setmetatable( {},mt)
					rawset(tb, key, ret)
				end
				return ret
			end,
			__indexTable = retmeta
		}
		return t
	end
	return setmetatable({},_default_metatable(rootmeta))
end
...
local function default_table(typename)
	...
	v = { 
		__index = function(tb, key)
			...
			if 'table' ~= type(ret) then
				-- add by Augustine 7/13/2018,
				rawset(tb, key, ret)
				return ret
			end 
			local retmeta = ret
			ret = setdefaultmetatable(retmeta)
			--[[ret = setmetatable({}, { 
					__index = function(tb1, key1)
					local ret1 = retmeta[key1] 
					--dump(retmeta)
					--print(key1)

					if 'table' ~= type(ret1) then
						rawset(tb1, key1, ret1)
					end
...
local decode_message_mt = {needexpend = 1}

local function decode_message_cb(typename, buffer)
	return setmetatable ( { typename, buffer } , decode_message_mt)
end

local function get_msg_enums(typename)
	local tEnums = _msg_enums_cache[typename]
	if tEnums == nil then
		tEnums = c._peek_message_enums(P,typename)
		_msg_enums_cache[typename] = tEnums
	end
	return tEnums
end

local function get_msg_Type(tbl)
	...
	if type(tbl) == "table" then
		...
		if mt ~= nil then
			-- need expend
			if (#tbl) == 2 and  rawget(mt,"needexpend") ~= nil then
				szType = tbl[1]
			else
				-- defalut
				local typetab = rawget(mt,"__indexTable")
				if typetab ~= nil and type(typetab) == "table"then
					szType = rawget(typetab,"_CType")
				end
				if type(szType) ~= "string" or (#tbl) ~= 0 then
					szType = nil
				end
...
local function deep_foreach(tpb,typename)
	
	local function deep_foreach_Imp(tpb,typename)
		
		if typename ~= nil then
			local tEnums = get_msg_enums(typename)
			local t
			for k,v in pairs(tEnums) do
			-- 就是访问下，让其expand和default打开
				t = tpb[k]
				local tt = get_msg_Type(t)
				if tt ~= nil then
					deep_foreach_Imp(t,tt)
				elseif type(t) == "table" then 
					deep_foreach_Imp(t,nil)
				end
			end
		else
			for k,v in pairs(tpb) do
				local tt = get_msg_Type(v)
				if tt ~= nil then
					deep_foreach_Imp(v,tt)
				elseif type(t) == "table" then 
					deep_foreach_Imp(v,nil)
				end
			end
		end
	
	end
	deep_foreach_Imp(tpb,typename)
end

function readnil_ignore_assert()
	--add by Augustine 9/29/2018
	_reader[0] = function(msg) return _reader.null end
	---------------------------
end
...
function decode(typename, buffer,...)
	...
	local ok
	if arg1 == nil then 
		ok = c._decode(P, decode_message_cb , ret , typename, buffer)
	elseif arg2 == nil then
		ok = c._decode(P, decode_message_cb , ret , typename, buffer,arg1)
	else
		ok = c._decode(P, decode_message_cb , ret , typename, buffer,arg1,arg2)
	end
	if ok then
		setmetatable(ret , default_table(typename))
		deep_foreach(ret,typename)
		return ret
	else
		assert(false, "Proto:"..typename.." "..c._last_error(P))
